...
class PdfFile:
    ...
    async def parse(self):
        ...
        response = requests.get(self.url_parse1, headers=headers, verify=False)
        response.raise_for_status()
        file_name = f'{os.getcwd()}/banks/open/data/{uuid.uuid4()}.pdf'
        with open(file_name, "wb") as file:
            file.write(response.content)
        await asyncio.sleep(1)
        self.titles.append("СБОРНИК ТАРИФОВ Публичного акционерного общества Банк «Финансовая Корпорация Открытие»")
    ...
    async def compare(self):
        ...
        changes_from_db = await AsyncORM.select_changes_for_compare(
            bank=Banks.open, typechanges=TypeChanges.pdf_file, lim=4)
