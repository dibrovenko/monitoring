...
class PdfFile:
    ...
    async def parse(self):
        link_new_file = await selenium_pdf_down(url=self.url_parse1, download_path=f'{os.getcwd()}/banks/modul/data')
        if not os.path.exists(link_new_file):
            link_new_file = await selenium_pdf_down(url=self.url_parse1, download_path=f'{os.getcwd()}/banks/modul/data')
        if not os.path.exists(link_new_file):
            link_new_file = await selenium_pdf_down(url=self.url_parse1, download_path=f'{os.getcwd()}/banks/modul/data')
        if os.path.exists(link_new_file):
            self.titles.append("ТАРИФЫ ДЛЯ «СЕЛЛЕРОВ»")
        ...
        file_name = f'{os.getcwd()}/banks/modul/data/{uuid.uuid4()}.pdf'
        with open(file_name, "wb") as file:
            file.write(response.content)
        await asyncio.sleep(1)
        self.titles.append("СБОРНИК ТАРИФОВ АО КБ «МОДУЛЬБАНК»")
    ...
    async def compare(self):
        ...
        changes_from_db = await AsyncORM.select_changes_for_compare(
            bank=Banks.modul, typechanges=TypeChanges.pdf_file, lim=4)
